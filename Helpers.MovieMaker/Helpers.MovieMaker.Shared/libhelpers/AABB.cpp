...
AABB::AABB(const D2D1_RECT_F &bbox) {
	...
	this->min = { this->center.x - this->radius.x, this->center.y - this->radius.y };
	this->max = { this->center.x + this->radius.x, this->center.y + this->radius.y };
}

AABB::AABB(std::vector<DirectX::XMFLOAT2> vecPoints) {
	...
	this->radius = { (maxX - minX) / 2, (maxY - minY) / 2 };
	this->center = { minX + this->radius.x, minY + this->radius.y };
}
...
bool AABB::DoCollide(const AABB &rect) const {
