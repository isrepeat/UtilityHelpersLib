...
namespace H {
	...
	::AABB Math::GetAABBFromRotatedPoints(float radAngle, std::vector<DirectX::XMFLOAT2> vecPoints) {
		auto matrixRotZ = DirectX::XMMatrixRotationZ(radAngle);
		   
		std::vector<DirectX::XMFLOAT2> rotatedPoints;
		for (const auto& point : vecPoints) {
			auto rotatedPoint = DirectX::XMVector3Transform(XMLoadFloat2(&point), matrixRotZ);
			   
			rotatedPoints.push_back({});
			XMStoreFloat2(&rotatedPoints.back(), rotatedPoint);
		}
		return ::AABB{ rotatedPoints };
	}
	...
	D2D1_RECT_F Math::TransformRect(const D2D1_RECT_F &rect, const D2D1_MATRIX_3X2_F &matrix) {
		...
		transformed = Math::SortRect(transformed);
	...
	bool Math::AreaInsideOfArea(const DirectX::XMFLOAT2& container, const DirectX::XMFLOAT2& insider) {
		auto cContainer = container.x * container.x + container.y * container.y;
		auto cInsider = insider.x * insider.x + insider.y * insider.y;

		return cContainer <= cInsider;
	}

	bool Math::XMVector3IsNull(DirectX::XMVECTOR & vec) {
		return vec.XF == 0.0f || vec.YF == 0.0f || vec.ZF == 0.0f;
	}

	DirectX::XMMATRIX Math::XMMatrixPerspectiveLH(
		float ViewWidth,
		float ViewHeight,
		float ViewZ,
		float NearZ,
