...
#include "HSystem.h"
#include "Filesystem\IWinStreamExt.h"
...
uint32_t ImageUtils::GetFrameRectByteSize(IWICBitmapSource* frame, const WICRect& segmentArea) const {
    uint32_t byteSize = 0;
    auto pixelBitSize = this->GetPixelBitSize(frame);

    byteSize = (static_cast<uint32_t>(std::abs(segmentArea.Width)) * static_cast<uint32_t>(std::abs(segmentArea.Height))) * (pixelBitSize / 8);

    return byteSize;
}

uint32_t ImageUtils::GetFrameRectStride(IWICBitmapSource* frame, const WICRect& segmentArea) const {
    uint32_t stride = 0;
    auto pixelBitSize = this->GetPixelBitSize(frame);

    // formula from https://msdn.microsoft.com/en-us/library/windows/desktop/ee690179%28v=vs.85%29.aspx Codec Developer Remarks
    stride = (static_cast<uint32_t>(std::abs(segmentArea.Width)) * pixelBitSize + 7) / 8;
...
DirectX::XMUINT2 ImageUtils::GetFrameRotatedSize(IWICBitmapSource *frame) const {
    WICBitmapTransformOptions rotateFlip;
    DirectX::XMUINT2 frameSize = this->GetFrameSize(frame);

    try {
        Microsoft::WRL::ComPtr<IWICBitmapFrameDecode> decodeFrame;
        frame->QueryInterface(IID_PPV_ARGS(&decodeFrame));
        auto exifRotation = this->GetExifRotation(decodeFrame.Get());
        rotateFlip = this->GetInversedRotationFlipOptions(exifRotation);
    }
    catch (...) {
        rotateFlip = WICBitmapTransformOptions::WICBitmapTransformRotate0;
    }

    if ((rotateFlip & WICBitmapTransformOptions::WICBitmapTransformRotate90) ||
        (rotateFlip & WICBitmapTransformOptions::WICBitmapTransformRotate270))
    {
...
void ImageUtils::DecodePixels(IWICBitmapSource *frame, uint32_t pixelsByteSize, void *pixels, const WICRect *rect) const {
    ...
    if (rect) {
        frameStride = this->GetFrameRectStride(frame, *rect);
    }
    else {
...
Microsoft::WRL::ComPtr<IWICBitmapEncoder> ImageUtils::CreateEncoder(
    const Microsoft::WRL::ComPtr<IStream> &stream,
    ImageContainer format) const
{
    const GUID_ContainerFormat *fmt = nullptr;
    ...
    if (fmt) {
        return this->CreateEncoder(stream, *fmt);
    }
...
Microsoft::WRL::ComPtr<IWICBitmapScaler> ImageUtils::Scale(
    ...
    WICBitmapInterpolationMode interpolationMode) const
{
    HRESULT hr = S_OK;
    Microsoft::WRL::ComPtr<IWICBitmapScaler> transformedFrame;

    auto tmpSize = destSize;
    ...
    hr = transformedFrame->Initialize(frame, tmpSize.x, tmpSize.y, interpolationMode);
    H::System::ThrowIfFailed(hr);
...
ImagePixels ImageUtils::LoadDxPBgra32Image(Filesystem::IStream *stream, uint32_t maxSize) {
    Microsoft::WRL::ComPtr<IStream> istream;
    auto winStreamExt = dynamic_cast<Filesystem::IWinStreamExt *>(stream);

    winStreamExt->CreateIStream(istream.GetAddressOf());

    return ImageUtils::LoadDxPBgra32Image(istream, maxSize);
}
...
ImagePixels ImageUtils::LoadDxPBgra32Image(const Microsoft::WRL::ComPtr<IWICBitmapDecoder> &decoder, uint32_t maxSize) {
    HRESULT hr = S_OK;
    ImagePixels pixels;
    ImageUtils imgUtils;
    WICBitmapTransformOptions rotateFlip;
    Microsoft::WRL::ComPtr<IWICBitmapSource> frame = imgUtils.CreateFrameForDecode(decoder.Get());
...
Microsoft::WRL::ComPtr<ID2D1Bitmap1> ImageUtils::LoadBmpFromPath(const Microsoft::WRL::ComPtr<IStream> &stream,
    ...
    Microsoft::WRL::ComPtr<IWICFormatConverter> newFrame = imgUtils.ConvertPixelFormatInMemory(imgDecoderFrame.Get(), GUID_WICPixelFormat32bppPBGRA);
    ...
    HRESULT hres = d2dCtx->CreateBitmapFromWicBitmap(newFrame.Get(), &bitmapProps, res.GetAddressOf());
    H::System::ThrowIfFailed(hres, L"LoadBmp from ImgUtils");
