...
//#include "../../MovieMakerCore/MovieMakerCore/Enums/ShiftTypes.h"
...
public:
    ...
    static void Set(Json::Value& obj, std::string name, T &&v) {
        obj[name] = Json::Value(HJsonNative::Serialize(v));
    }
...
#if HAVE_WINRT == 1
    static Json::Value Serialize(const Windows::Foundation::Size &v);
    static Json::Value Serialize(const Windows::Foundation::Point &v);
#endif
    ...
    // helper for Set to be used with Json::Value&
    static const Json::Value& Serialize(const Json::Value& val);
    template<class T>
    static Json::Value Serialize(const std::chrono::duration<T>& v);
    template<class T>
    static Json::Value Serialize(const std::vector<T>& v);
    template <typename T, size_t N>
    static Json::Value Serialize(const T(&v)[N]);


    template<class T>
    static void Get(const Json::Value& obj, std::string name, T &v) {
        HJsonNative::Deserialize(Json::Value(obj[name]), v);
    }
...
#if HAVE_WINRT == 1
    static bool Deserialize(const Json::Value& json, Windows::Foundation::Size& out);
    static bool Deserialize(const Json::Value& json, Windows::Foundation::Point& out);
#endif
    ...
    // helper for Get to be used with Json::Value&
    static bool Deserialize(const Json::Value& json, Json::Value& val);

    template<class T>
    static bool Deserialize(const Json::Value& json, std::chrono::duration<T>& v);

    template<class T>
    static bool Deserialize(const Json::Value& json, std::vector<T>& out);

    template <typename T, size_t N>
    static bool Deserialize(const Json::Value& json, T(&out)[N]);
};

template<class T>
inline Json::Value HJsonNative::Serialize(const std::chrono::duration<T>& v) {
    return HJsonNative::Serialize(v.count());
}

template<class T>
inline Json::Value HJsonNative::Serialize(const std::vector<T>& v) {
    Json::Value res;

    for (auto& i : v) {
        res.append(Json::Value(HJsonNative::Serialize(i)));
    }
...
template <typename T, size_t N>
inline Json::Value HJsonNative::Serialize(const T(&v)[N]) {
    Json::Value res;

    for (size_t i = 0; i < N; i++) {
        res.append(Json::Value(HJsonNative::Serialize(v[i])));
    }
...
template<class T>
inline bool HJsonNative::Deserialize(const Json::Value& json, std::chrono::duration<T>& v) {
    T tmpVal = {};

    if (!HJsonNative::Deserialize(json, tmpVal)) {
        return false;
    }
...
template<class T>
inline bool HJsonNative::Deserialize(const Json::Value& json, std::vector<T>& out) {
    bool res = true;
    if (json.isArray()) {
        for (auto& i : json) {
            T tmp;
            res = res && HJsonNative::Deserialize(i, tmp);
            out.push_back(tmp);
        }
        return res;
    }
...
template <typename T, size_t N>
inline bool HJsonNative::Deserialize(const Json::Value& json, T(&out)[N]) {
    bool res = true;
    if (json.isArray()) {
        for (size_t i = 0; i < N; i++) {
            T tmp;
            res = res && HJsonNative::Deserialize(json[i], tmp);
            out[i] = tmp;
        }
        return res;
    }
