...
namespace HELPERS_NS {
	namespace Win32 {
		...
		void MainWindow::RunMessageLoop() {
			...
			// GetMessage returns:
			//  > 0 Ч a message was retrieved (msg filled), continue processing
			//    0 Ч WM_QUIT received, time to exit the message loop
			//   -1 Ч an error occurred (invalid HWND, system failure, etc.)
		...
		LRESULT MainWindow::HandleMessage(UINT message, WPARAM wParam, LPARAM lParam) {
			for (const auto& messageHook : this->messageHooks) {
				if (messageHook(message, wParam, lParam)) {
					return 0; // —ообщение обработано хук-функцией
				}
			}
