...
namespace STD_EXT_NS {
	...
	struct ifstream : public ::std::ifstream {
		...
		::std::optional<UTFByteOrderMark> DetectUTFByteOrderMark() {
			...
			// „итаем максимум N байт (по самой длинной сигнатуре BOM)
			size_t maxLength = 0;
	...
	private:
		void SkipUTFByteOrderMark() {
			...
			this->seekg(static_cast<std::streamoff>(bom.size()), std::ios::beg); // ѕропускаем UTF BOM
