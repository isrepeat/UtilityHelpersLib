...
namespace Dx {
	namespace WinRt {
		...
		void SwapChainPanel::CreateDeviceIndependentResources() {
			HRESULT hr = S_OK;
		...
		void SwapChainPanel::CreateDeviceResources() {
		...
		void SwapChainPanel::CreateWindowSizeDependentResources() {
				...
				m_swapChainPanel->Dispatcher->RunAsync(Windows::UI::Core::CoreDispatcherPriority::High, ref new Windows::UI::Core::DispatchedHandler([=]()
					{
						...
						hr = reinterpret_cast<IUnknown*>(m_swapChainPanel)->QueryInterface(IID_PPV_ARGS(panelNative.GetAddressOf()));
						H::System::ThrowIfFailed(hr);
			...
			hr = m_d3dDevice->CreateTexture2D1(
				&depthStencilDesc,
				nullptr,
				&depthStencil
			);
		...
		void SwapChainPanel::UpdateRenderTargetSize() {
			...
			if (!DisplayMetrics::SupportHighResolutions && m_dpi > DisplayMetrics::DpiThreshold)
			{
				...
				if (std::max(width, height) > DisplayMetrics::WidthThreshold && std::min(width, height) > DisplayMetrics::HeightThreshold)
				{
					...
					float scale = 1.0f;
					m_effectiveDpi /= scale;
					m_effectiveCompositionScaleX /= scale;
					m_effectiveCompositionScaleY /= scale;
				}
			}
		...
		void SwapChainPanel::SetSwapChainPanel(Windows::UI::Xaml::Controls::SwapChainPanel^ panel) {
		...
		void SwapChainPanel::RegisterDeviceNotify(Dx::WinRt::IDeviceNotify* deviceNotify) {
